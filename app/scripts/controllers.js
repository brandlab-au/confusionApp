...
//getDishes is  fucntion();
$scope.dishes= menuFactory.getDishes();        

          //$scope.dishes = dishes;
          $scope.select = function(setTab){ $scope.tab = setTab ;
...
.controller('DishCommentController',['$scope',function($scope){
  ...
  $scope.feed = {rating:"", comment:"", author:"", date:$scope.date };

  // Step 3: Push your comment into the dish's comment array
    //$scope.dish.comments.push($scope.feed);

    $scope.submit = function() {
      //console.log($scope.feed);
       {
        $scope.feed.date = new Date().toISOString();
        $scope.dish.comments.push($scope.feed);
        $scope.list.push($scope.date);
